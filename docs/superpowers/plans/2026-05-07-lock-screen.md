...
- [x] **Step 1：在 `_check.js` 末尾、`window._runChecks` 之前加入 Section 5 測試函式**
...
- [x] **Step 2：在 `window._runChecks` 內的單元測試區段加入 testLockScreen 呼叫**
...
- [x] **Step 3：在 `index.html` 的 LS 物件加入兩個新 key**
...
- [x] **Step 4：在 `index.html` 的 `<script>` 標籤內、LS 物件之後加入三個純函式**
...
- [x] **Step 5：在瀏覽器開啟 index.html，Console 執行 `window._runChecks()`**
...
- [x] **Step 1：在 `</style>` 標籤（約第 219 行）之前插入鎖定畫面樣式**
...
- [x] **Step 2：在瀏覽器重整 index.html，確認沒有 CSS 語法錯誤（頁面正常顯示）**
...
- [x] **Step 1：在 `<div class="alert-banner"...>` 之後（約第 237 行）插入鎖定畫面 HTML**
...
- [x] **Step 2：在 Header 的 `<div class="timestamp" id="clock"></div>` 之前（約第 233 行）加入鎖定按鈕**
...
- [x] **Step 3：在瀏覽器重整，確認頁面正常顯示（lock-screen 不應出現，因為尚未整合 JS）**
...
- [x] **Step 1：在 `// ── INIT ───` 這行之前插入 UI 與狀態函式**
...
- [x] **Step 2：Commit（不含 init 整合，先保持可用狀態）**
...
- [x] **Step 1：在 Task 4 插入的 `confirmUnlock` 函式之後，`// ── INIT` 之前，插入閒置計時器函式**
...
- [x] **Step 1：在 init() 最頂端加入鎖定事件綁定與鎖定檢查**
...
- [x] **Step 4：驗證「Header 鎖定按鈕」**

1. 解鎖狀態下點 Header 的「🔒 鎖定」按鈕
2. 預期：立即顯示解鎖畫面

- [x] **Step 5：驗證「OAuth redirect 相容性」**
...
- [x] **Step 1：確認 _check.js 的 `window._runChecks` 有正確 `await` testLockScreen**
...
- [x] **Step 2：在瀏覽器開啟 index.html，解鎖後在 Console 執行 `window._runChecks()`**
...
- [x] **Step 3：在 DEVELOPMENT-PLAN.md 加入 Phase 9**
